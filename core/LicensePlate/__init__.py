...
from .yolo import yolo_LP_detect, yolo_license_plate
from .function import helper
from .function import utils_rotate as utils_rotate

import os
current_directory = os.path.dirname(__file__)
cropImgPath = f'{current_directory}/_test/crop.jpg'
frameImgPath = f'{current_directory}/_test/frame.jpg'
cropRedChannelPath = f'{current_directory}/_test/crop-red.jpg'
cropGreenChannelPath = f'{current_directory}/_test/crop-green.jpg'
cropBlueChannelPath = f'{current_directory}/_test/crop-blue.jpg'
cropCandidatePath = f'{current_directory}/_test/crop-candidate.jpg'

cropPlatePath = f'{current_directory}/_test/plate_cropped.jpg'
cropPlatePerspectivePath = f'{current_directory}/_test/plate_rotated_perspective.jpg'
...
def four_point_transform(image, quad, force_width=None, force_ar=None):
    ...
    if force_width is not None: W = int(force_width)
...
def normalizePlate(image):
    ...
    if best_quad is None:
        candidates = sorted(
            ...
            key=lambda x: sum(cv2.contourArea(c) for c in x[1]), reverse=True
        )
...
def recognizeLicensePlate(
    ...
    list_plates = yolo_LP_detect(img, size=640).pandas().xyxy[0].values.tolist()
    count = 0
...
def recognizeLicensePlateBuffer(
    source
):
